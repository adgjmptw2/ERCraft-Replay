...
def build_catalog(config: AnalysisConfig) -> dict:
    ...
    return {
        ...
        "dataSources": {
            ...
            "bserBackend": {
                "baseUrl": "https://bser-rest-release.bser.io",
                "gameResultEndpoint": "/api/battle/game/{gameId}",
                "replayLookupEndpoint": "/api/external/findReplayGame/{gameId}/{userNum}",
                "replayPathField": "rst.replayPath",
                "status": "observed-in-acquisition-code-not-live-crosschecked",
                "liveCrosscheck": "not-run-no-session-state",
                "role": "authenticated game result lookup and signed replay-object URL resolution",
            },
        },
        "map": {
        ...
        "limitations": [
            ...
            "BSER /api/battle/game 응답은 취득 코드에 존재하지만 현재 세션 상태가 없어 live field-by-field 비교는 실행하지 않았다.",
