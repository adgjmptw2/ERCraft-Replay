...
- 이미 확보한 분석 입력. 파일 획득 코드와 절차는 악용 가능성을 줄이기 위해 공개하지 않음
...
각 교전 카드에는 경기·선수·교전 구간이 일치하는 이벤트 추정 합계를 바탕으로 가한 피해와 받은 피해를 표시한다. 미확정 이벤트 건수는 별도로 표시하며, 확인된 합계가 0이고 미확정 이벤트가 있으면 `—`로 표시한다. 미확정 이벤트가 없는 합계 0은 `0`으로 표시한다. 경기 최종 총합의 잔차를 개별 교전에 나눠 넣지 않는다. 개별 교전의 오차가 5% 이내임을 보증하는 수치는 아니다.

이미 저장된 단일 경기의 비공개 분석과 `ercraft-practical-encounters.v1` 결과를 아래처럼 변환한다. 공개 분석 파일은 뷰어가 읽는 파일과 바이트까지 같아야 한다. 출력 파일은 기존 파일을 덮어쓰지 않으며, 원본 입력과 비공개 분석은 공개 경로에 넣지 않는다.

```powershell
cd core
..\.venv\Scripts\python -m decoder.practical_encounter_damage PUBLIC_JSON PRIVATE_ANALYSIS_JSON PRACTICAL_JSON OUTPUT_JSON
cd ..
```

생성된 출력만 뷰어의 `public/ui-assets/encounter-damage-v1.json`으로 연결한다. 워커에 통합할 때는 `build_sidecars(..., practical_damage=저장된_결과)`로 전달할 수 있다. 입력 제공은 호출부에서 별도로 연결해야 하며 자동 생성이나 운영 배포가 포함된 것은 아니다. 파일이 없거나 경기·구간 검증에 실패하면 카드에 `미확인`으로 표시한다.

검증은 저장된 실제 경기의 24명·310개 교전에서 620개 피해량 항목의 변환 및 카드 표시 모델 연결을 확인했다. 전체 브라우저 화면이나 개별 교전 피해량 자체의 정확도를 검증한 결과와는 구분한다.
